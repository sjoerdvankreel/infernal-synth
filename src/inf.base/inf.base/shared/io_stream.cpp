...
bool operator<(old_param_id const& l, old_param_id const& r)
{ return l.part_guid < r.part_guid && l.part_index < r.part_index && l.param_guid < r.param_guid; }
...
io_stream::load(topology_info const& topology, param_value* state)
{
  ...
  std::set<old_param_id> unmatched_parameters;
  for (std::int32_t sp = 0; sp < param_count; sp++)
  {
    ...
    unmatched_parameters.insert(old_id);
    ...
    for (std::int32_t rp = 0; rp < topology.input_param_count; rp++)
    {
      ...
      // Matched this one.
      old_param_id matched_old_id;
      matched_old_id.part_guid = part_guid;
      matched_old_id.part_index = type_index;
      matched_old_id.param_guid = param_guid;
      unmatched_parameters.erase(matched_old_id);
  ...
  for (auto it = unmatched_parameters.begin(); it != unmatched_parameters.end(); ++it)
