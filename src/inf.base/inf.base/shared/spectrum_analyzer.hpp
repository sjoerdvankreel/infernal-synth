...
class spectrum_analyzer
{
  ...
  float power_at_note(std::vector<std::complex<float>> const& fft, std::int32_t midi) const;

public:
  spectrum_analyzer();
  static inline std::int32_t const bucket_count = 144;
  float const* analyze(std::vector<float> const& audio);
  float const* analyze(float const* audio, std::size_t count);
};
