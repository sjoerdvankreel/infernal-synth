...
// Hopefully enough.
inline std::int32_t const max_note_input_events = 1024;
inline std::int32_t const max_automation_events = 16384;
...
// Internally we build up automation curve from time stamped event stream.
...
struct block_input
{
  ...
  std::array<automation_event, max_automation_events> automation_events = {};
  std::array<note_input_event, max_note_input_events> note_input_events = {};
};
