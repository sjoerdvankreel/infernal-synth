...
reverb_comb_pos(), reverb_comb(), reverb_allpass_pos(), reverb_allpass()
{
  ...
  float fmax_delay_multi_hold_length = std::ceil(sample_rate * effect_dly_multi_hold_max_time_sec);
  std::int32_t max_delay_length = static_cast<std::int32_t>(fmax_delay_length);
  std::int32_t max_delay_multi_hold_length = static_cast<std::int32_t>(fmax_delay_multi_hold_length);
  delay_buffer[0] = base::ring_buffer<float>(max_delay_length + max_delay_multi_hold_length);
  delay_buffer[1] = base::ring_buffer<float>(max_delay_length + max_delay_multi_hold_length);
