...
cv_bank_graph::process_dsp_core(block_input const& input, float* output, float sample_rate)
{
  ...
  std::fill(cv_state.midi_pb.data(), cv_state.midi_pb.data() + input.data.sample_count, 0.0f);
  std::fill(cv_state.midi_cp.data(), cv_state.midi_cp.data() + input.data.sample_count, 1.0f);
