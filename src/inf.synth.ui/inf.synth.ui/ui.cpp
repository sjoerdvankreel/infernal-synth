...
{
  auto grid = create_grid_ui(controller, vaudio_bank_route_count + 1, 14);
  grid->add_cell(create_label_ui(controller, "In", Justification::centred), 0, 0, 1, 3);
  grid->add_cell(create_label_ui(controller, "Out", Justification::centred), 0, 3, 1, 3);
  grid->add_cell(create_label_ui(controller, "Gain", Justification::centred), 0, 6, 1, 4);
  grid->add_cell(create_label_ui(controller, "Bal", Justification::centred), 0, 10, 1, 4);
  for (std::int32_t i = 0; i < vaudio_bank_route_count; i++)
  {
    ...
    auto amt = grid->add_cell(create_param_edit_ui(controller, part_type, 0, audio_bank_param_index(i, audio_bank_param_type::amt), edit_type::hslider, tooltip_type::value, false, true), i + 1, 6, 1, 4);
    ...
    auto bal = grid->add_cell(create_param_edit_ui(controller, part_type, 0, audio_bank_param_index(i, audio_bank_param_type::bal), edit_type::hslider, tooltip_type::value, false, true), i + 1, 10, 1, 4);
