...
create_env_main_group(plugin_controller* controller, std::int32_t part_index)
{
  ...
  grid->add_cell(create_labeled_param_ui(controller, part_type::venv, part_index, envelope_param::on, edit_type::toggle, label_type::label, false), 5, 0);
  return create_part_group_ui(controller, create_group_label_ui(controller, "Main", false), std::move(grid));
}
...
create_env_adr_group(
  ...
  std::int32_t tempo2, std::int32_t slope1, std::int32_t slope2, std::int32_t split)
{
  ...
  slider_grid->add_cell(create_labeled_param_ui(controller, part_type::venv, part_index, time1, edit_type::vslider, label_type::label, true), 0, 2);
...
{
  auto grid = create_grid_ui(controller, 5, 7);
  grid->add_cell(create_part_group_container_ui(controller, create_env_main_group(controller, part_index)), 0, 0, 5, 1);
  grid->add_cell(create_part_group_container_ui(controller, create_env_adr_group(controller, part_index, "DA", 
    ...
    envelope_param::attack2_slope, envelope_param::attack_split_level)), 2, 1, 3, 2);
  grid->add_cell(create_part_group_container_ui(controller, create_env_adr_group(controller, part_index, "HD",
    ...
    envelope_param::decay2_slope, envelope_param::decay_split_level)), 2, 3, 3, 2);
  grid->add_cell(create_part_group_container_ui(controller, create_env_adr_group(controller, part_index, "SR",
    ...
    envelope_param::release2_slope, envelope_param::release_split_level)), 2, 5, 3, 2);
