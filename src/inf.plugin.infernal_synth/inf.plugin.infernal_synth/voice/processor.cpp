...
_vamp_bal(topology, part_type::vamp, sample_rate), _vcv_bank(topology, cv_state, gcv_uni_hold_, gcv_bi_hold_, glfo_hold_, velo, midi, input), 
...
void voice_processor::prepare_port(voice_input const& input)
{
  ...
  std::int32_t voice_mode = input.voice_mode;
  ...
  for (std::int32_t p = 0; p < sample_count; p++)
  {
    ...
    if (_voice_start &&
      voice_mode == voice_mode::poly && (
        port_trig == voice_port_trig::note ||
        (port_trig == voice_port_trig::voice && !_new_voice_section)))
    ...
    if (p == new_midi_start_pos &&
      voice_mode != voice_mode::poly && (
        port_trig == voice_port_trig::note ||
        (port_trig == voice_port_trig::voice && !_voice_start)))
    { 
...
bool voice_processor::process(voice_input const& input, cpu_usage& usage)
{
  ...
  if (voice_mode != voice_mode::poly) retrig_pos = new_midi_start_pos;
  for (std::int32_t i = 0; i < venv_count; i++)
  {
  ...
  usage.vamp = amp_bal_out.own_time;
  return ended;
}  
