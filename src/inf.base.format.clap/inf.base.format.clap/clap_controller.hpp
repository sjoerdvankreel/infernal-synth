...
public:
  ...
  std::string preset_file_extension() { return ""; }
  std::string default_theme_path(std::string const& plugin_file) const override { return ""; }
  std::vector<inf::base::external_resource> themes(std::string const& plugin_file) const override { return {}; }
  std::unique_ptr<host_context_menu> host_menu_for_param_index(std::int32_t param_index) const override { return {}; }
  std::vector<inf::base::external_resource> factory_presets(std::string const& plugin_file) const override { return {}; }
