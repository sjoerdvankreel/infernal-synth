...
{  
  ...
  colors.drag.color = from_vst_color_name(attrs.getAttributeValue("drag-color"), desc);
  colors.marker.color = from_vst_color_name(attrs.getAttributeValue("marker-color"), desc);
  colors.inner.color = from_vst_color_name(attrs.getAttributeValue("inner-color"), desc);
  colors.outer.color = from_vst_color_name(attrs.getAttributeValue("outer-color"), desc);
  colors.light.color = from_vst_color_name(attrs.getAttributeValue("light-color"), desc);
  colors.shadow.color = from_vst_color_name(attrs.getAttributeValue("shadow-color"), desc);
...
rotary_knob::draw(VSTGUI::CDrawContext* context)
{
  ...
  // outer border
  context->setFrameColor(to_vst_color(_colors.outer.color));
  ...
  if(_discrete)
  {
    ...
    context->setFrameColor(to_vst_color(_colors.drag.color));
    context->drawArc(CRect(CPoint(3, 3), inner_size - CPoint(6, 6)), border_hi_start, border_hi_end, kDrawStroked);
    context->setFrameColor(to_vst_color(_colors.inner.color.darken(0.5f)));
  ...
  else if (!_bipolar)
  {
    ...
    context->setFrameColor(to_vst_color(_colors.drag.color));
    context->drawArc(CRect(CPoint(3, 3), inner_size - CPoint(6, 6)), border_hi_start, border_hi_end, kDrawStroked);
    context->setFrameColor(to_vst_color(_colors.inner.color));
  ...
  for (std::int32_t i = 0; i < light_parts; i++)
  {
    ...
    context->setFrameColor(CColor(_colors.light.color.r, _colors.light.color.g, _colors.light.color.b, alpha));
  ...
  for (std::int32_t i = 0; i < light_parts; i++)
  {
    ...
    context->setFrameColor(CColor(_colors.shadow.color.r, _colors.shadow.color.g, _colors.shadow.color.b, alpha));
    context->drawArc(CRect(CPoint(1, 1), inner_size - CPoint(2, 2)), angle1, angle2, kDrawStroked);
  }
  ...
  context->setFrameColor(to_vst_color(_colors.marker.color));
  context->drawLine(CPoint(center, center), point_on_circle(center, radius, theta));

  // spot markers
  if (_bipolar)
  {
    context->setFillColor(to_vst_color(angle >= 0.51f || angle <= 0.49f? _colors.marker.color : _colors.inner.color));
    context->drawEllipse(CRect(CPoint(outer_size.x / 2.0 - 3.0, 1.0), CPoint(3.0, 3.0)), kDrawFilled);
  }
  else if(!_discrete)
  {
    context->setFillColor(to_vst_color(angle >= 0.99f ? _colors.marker.color : _colors.inner.color));
    context->drawEllipse(CRect(point_on_circle(center, radius, start), CPoint(2.0, 2.0)), kDrawFilled);
    context->setFillColor(to_vst_color(angle >= 0.01f ? _colors.inner.color : _colors.marker.color));
    context->drawEllipse(CRect(point_on_circle(center, radius, -start), CPoint(2.0, 2.0)), kDrawFilled);
  }
} 
